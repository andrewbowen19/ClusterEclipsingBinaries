...
class analyseClusterLISA(object):
	...
	def analyse(self, path, clusterType,  strategy, crowding, searchWDs):
		...
		if self.searchWDs is True:
			...
			WDall.to_csv(self.path + f'data/wd/all-{self.clusterType}{self.strategy}{self.crowding}-WD-histDataLISA.csv', header = self.csv_cols)
			WDobs.to_csv(self.path + f'data/wd/obs-{self.clusterType}{self.strategy}{self.crowding}-WD-histDataLISA.csv', header = self.csv_cols)
			WDrec.to_csv(self.path + f'data/wd/rec-{self.clusterType}{self.strategy}{self.crowding}-WD-histDataLISA.csv', header = self.csv_cols)
