...
def saveHist(histAll, histObs, histRec, bin_edges, xtitle, fname):
	c1 = '#5687A6'  #darker blue from Dali
	c2 = '#8EB1BF' #lighter blue
	c3 = '#A62B1F' #Dali red
	...
	f.savefig('/projects/p30137/abowen/CEB/plots/'+fname+'.pdf',format='pdf', bbox_inches = 'tight')#saving figures
...
# Also, this weights those near the galactic plane sooo highly (and these are usually poorly recovered), that the resulting histograms are VERY noisy (since we're basically just looking at a few fields new to galactic plane)
		...
		if (Nall >= Nlim):
			...
			if (Nobs >= Nlim):
				m1hObs0, m1b = np.histogram(obs["m1"], bins=mbins)#Mass 1 hist
				qhObs0, qb = np.histogram(obs["m2"]/obs["m1"], bins=qbins)#Mass 2 hist
				ehObs0, eb = np.histogram(obs["e"], bins=ebins)#eccentricity histogram
				lphObs0, lpb = np.histogram(np.ma.log10(obs["p"].values).filled(-999), bins=lpbins)#log-period hist
				dhObs0, db = np.histogram(obs["d"], bins=dbins)#Distance hist (maybe diam)
				maghObs0, magb = np.histogram(obs["appMagMean_r"], bins=magbins)#magnitude hist
				rhObs0, rb = np.histogram(obs["r2"]/obs["r1"], bins=rbins)#radius ratio hist
				...
				#Rec
				fullP = abs(data['LSM_PERIOD'] - data['p'])/data['p']#full period difference
				halfP = abs(data['LSM_PERIOD'] - 0.5*data['p'])/(0.5*data['p'])#half period difference 
				twiceP = abs(data['LSM_PERIOD'] - 2.*data['p'])/(2.*data['p'])
				rec = data.loc[(data['LSM_PERIOD'] != -999) & ( (fullP < Pcut) | (halfP < Pcut) | (twiceP < Pcut))]#recovered periods
				Nrec = len(rec.index)#Number of recovered periods
