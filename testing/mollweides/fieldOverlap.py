...
class fieldOverlap(object):
	...
	def getClusterCoords(self):
		...
		self.gcRad = self.gcData['rhm[pc]'].values * u.pc
		self.ocRad = self.ocData['rhm[pc]'].values * u.pc
		# Converting to degrees (via parallax)
		self.gcRad_deg = (self.gcRad).to(u.deg, equivalencies=u.parallax())
		self.ocRad_deg = (self.ocRad).to(u.deg, equivalencies=u.parallax())
		# print(self.gcRad_deg, '\n', self.ocRad_deg)
	...
	def findNeighbors(self, strategy):
		...
		self.fieldRA = Angle(self.fieldDat['OpSim RA'].loc[np.where(self.fieldDat['Nobs'] > 0.)], unit = u.degree)
		self.fieldDec = Angle(self.fieldDat['OpSim Dec'].loc[np.where(self.fieldDat['Nobs'] > 0.)], unit = u.degree)
		...
		# Printing results
		print('########################################')
		print(r'% of GCs that overlap with multiple OpSim fields: ', multipleOverlapPecentGC * 100, 
				len(nOpSimOverlapsGC[np.where(nOpSimOverlapsGC > 1.)]),len(nOpSimOverlapsGC))
		print(r'% of OCs that overlap with multiple OpSim fields: ', multipleOverlapPecentOC * 100, 
				len(nOpSimOverlapsOC[np.where(nOpSimOverlapsOC > 1.)]), len(nOpSimOverlapsOC))
...
# self.gcRad_deg[np.where(nOpSimOverlapsGC > 1.)]
# *np.pi/180.
		# OC plots
...
for s in os.listdir('./output'):
	...
	print(s.replace('-OpSim-FieldData.csv',''))
