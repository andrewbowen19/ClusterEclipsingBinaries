...
if __name__ == "__main__":
	...
	d = os.path.join("/projects", "p30137", "ageller", "testing", "EBLSST", "clusters", "GlobularClusters", "output_files")
	files = os.listdir(d)
	IDs = []
	...
	peccAll.to_csv(os.path.join('.', 'pecc', 'all-GBN-ecc-p.csv'), header = ['e', 'p'])
	peccObs.to_csv(os.path.join('.', 'pecc', 'obs-GBN-ecc-p.csv'), header = ['e', 'p'])
	peccRec.to_csv(os.path.join('.', 'pecc', 'rec-GBN-ecc-p.csv'), header = ['e', 'p'])
