...
# Colossus Open Clusters script -- WITH` crowding
# White Dwarf (WD) version of analyse script
# Pulling LISA WD GW progenitor candidates
...
def saveHist(histAll, histObs, histRec, bin_edges, xtitle, fname, filters = ['u_', 'g_', 'r_', 'i_', 'z_', 'y_','all']):
	...
	ax1.set_ylabel('PDF')
	ax1.set_yscale('log')
	ax1.set_title('Open Clusters - Colossus (crowding)', fontsize = 16)
...
if __name__ == "__main__":
	...
	d = "/projects/p30137/ageller/testing/EBLSST/clusters/OpenClusters/withCrowding/colossus/output_files/"
	files = os.listdir(d)
	IDs = []
			...
			if (Nall >= Nlim):
				...
				if (Nobs >= Nlim):
					...
					for filt in filters:
						...
						# White dwarf appending
						prsaRecWD = data.loc[(data['p'] < 0.1) & (data['p'] > 0.0) & (data['LSM_PERIOD'] != -999)  & ((fullP < Pcut) | (halfP < Pcut) | (twiceP < Pcut))
							 & ((data['m1'] < 0.6) | (data['m2'] < 0.6))  & ((data['r1'] < wdMRrelation(data['m1'])) | (data['r2'] < wdMRrelation(data['m2'])))]
	...
	All.to_csv('./data/all-OCC-histData.csv', header = ['p', 'm1', 'm2', 'r1', 'r2', 'e', 'i', 'appMagMean_r'])
	Obs.to_csv('./data/obs-OCC-histData.csv', header = ['p', 'm1', 'm2', 'r1', 'r2', 'e', 'i', 'appMagMean_r'])
	Rec.to_csv('./data/rec-OCC-histData.csv', header = ['p', 'm1', 'm2', 'r1', 'r2', 'e', 'i', 'appMagMean_r'])
	...
	print('White Dwarf LISA Candidates: ', WDall, WDobs, WDrec)

	WDall.to_csv('./data/wd/all-OCC-WD-LISA-histData.csv', header = ['p', 'm1', 'm2', 'r1', 'r2', 'e', 'i', 'appMagMean_r'])
	WDobs.to_csv('./data/wd/obs-OCC-WD-LISA-histData.csv', header = ['p', 'm1', 'm2', 'r1', 'r2', 'e', 'i', 'appMagMean_r'])
	WDrec.to_csv('./data/wd/rec-OCC-WD-LISA-histData.csv', header = ['p', 'm1', 'm2', 'r1', 'r2', 'e', 'i', 'appMagMean_r'])
