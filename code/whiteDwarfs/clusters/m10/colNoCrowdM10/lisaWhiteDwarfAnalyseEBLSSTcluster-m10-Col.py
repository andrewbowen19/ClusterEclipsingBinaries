...
# Colossus M10 (GC) long script -- NO crowding
...
def saveHist(histAll, histObs, histRec, bin_edges, xtitle, fname, filters = ['u_', 'g_', 'r_', 'i_', 'z_', 'y_','all']):
	...
	ax1.set_ylabel('PDF')
	ax1.set_yscale('log')
	ax1.set_title('M10 - Colossus', fontsize = 16)
...
if __name__ == "__main__":
			...
			if (Nall >= Nlim):
				...
				if (Nobs >= Nlim):
					...
					for filt in filters:
						...
						# White dwarf appending
						prsaRecWD = data.loc[(data['p'] < 0.1) & (data['p'] > 0.001) & (data['LSM_PERIOD'] != -999)  & ((fullP < Pcut) | (halfP < Pcut) | (twiceP < Pcut))
							 & ((data['m1'] < 0.6) | (data['m2'] < 0.6))  & ((data['r1'] < wdMRrelation(data['m1'])) | (data['r2'] < wdMRrelation(data['m2'])))]
	...
	All.to_csv('./data/all-M10CN-histData.csv', header = ['p', 'm1', 'm2', 'r1', 'r2', 'e', 'i', 'appMagMean_r'])
	Obs.to_csv('./data/obs-M10CN-histData.csv', header = ['p', 'm1', 'm2', 'r1', 'r2', 'e', 'i', 'appMagMean_r'])
	Rec.to_csv('./data/rec-M10CN-histData.csv', header = ['p', 'm1', 'm2', 'r1', 'r2', 'e', 'i', 'appMagMean_r'])
	...
	WDall.to_csv('./data/wd/all-M10CN-WD-LISA-histData.csv', header = ['p', 'm1', 'm2', 'r1', 'r2', 'e', 'i', 'appMagMean_r'])
	WDobs.to_csv('./data/wd/obs-M10CN-WD-LISA-histData.csv', header = ['p', 'm1', 'm2', 'r1', 'r2', 'e', 'i', 'appMagMean_r'])
	WDrec.to_csv('./data/wd/rec-M10CN-WD-LISA-histData.csv', header = ['p', 'm1', 'm2', 'r1', 'r2', 'e', 'i', 'appMagMean_r'])
