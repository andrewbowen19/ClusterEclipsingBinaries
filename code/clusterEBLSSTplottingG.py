...
def saveHist(histAll, histObs, histRec, colAll,colObs, colRec, bin_edges, xtitle, fname, filters = ['u_', 'g_', 'r_', 'i_', 'z_', 'y_','all']):
	...
	# Plotting colossus data
	ax1.step(bin_edges, colAll/np.sum(colAll), color=c1)
	ax1.step(bin_edges, colObs/np.sum(colObs), color=c2)
	ax1.step(bin_edges, colRec[f]/np.sum(colRec[f]), color=c3, linewidth=lw, linestyle = '--')
	ax1.set_ylabel('PDF')
	ax1.set_yscale('log')
	ax1.set_title('Globular Clusters', fontsize = 16)
	ax1.set_xlabel(xtitle)

	#CDF - not needed for AAS poster
	#cdfAll = []
	#cdfObs = []
	#cdfRec = dict()
	#for f in filters:
...
if __name__ == "__main__":
	...
	#####################
			...
			if (Nall >= Nlim):
				...
				qcAll0, qb = np.histogram(data["m2"]/data["m1"], bins=qbins)
				...
				if (doIndividualPlots):
					...
					axqrat.step(qb[0:-1], qcAll0/np.sum(qcAll0), color='black', alpha=0.1)
...
if __name__ == "__main__":
	...
	#####################
			...
			if (Nall >= Nlim):
				#create histograms
				#All
				m1cAll0, m1b = np.histogram(data["m1"], bins=mbins)
				qcAll0, qb = np.histogram(data["m2"]/data["m1"], bins=qbins)
				ehAll0, eb = np.histogram(data["e"], bins=ebins)
				lphAll0, lpb = np.histogram(np.ma.log10(data["p"].values).filled(-999), bins=lpbins)
				dhAll0, db = np.histogram(data["d"], bins=dbins)
				...
				if (doIndividualPlots):
					axmass.step(m1b[0:-1], m1cAll0/np.sum(m1cAll0), color='black', alpha=0.1)
					axqrat.step(qb[0:-1], qcAll0/np.sum(qcAll0), color='black', alpha=0.1)
					axecc.step(eb[0:-1], ehAll0/np.sum(ehAll0), color='black', alpha=0.1)
					axlper.step(lpb[0:-1], lphAll0/np.sum(lphAll0), color='black', alpha=0.1)
					axdist.step(db[0:-1], dhAll0/np.sum(dhAll0), color='black', alpha=0.1)
				...
				dm1 = np.diff(m1b)
				m1val = m1b[:-1] + dm1/2.
				fb = np.sum(m1cAll0/len(data.index)*fbFit(m1val))
				#account for the hard-soft boundary
				Phs = getPhs(header['clusterVdisp'].iloc[0]*units.km/units.s).to(units.day).value
				fb *= RagNormal(np.log10(Phs), cdf = True)
				print("fb, Phs = ", fb, Phs)
				...
				m1cAll += m1cAll0/Nall*Nmult
				qcAll += qcAll0/Nall*Nmult
				ehAll += ehAll0/Nall*Nmult
				lphAll += lphAll0/Nall*Nmult
				dhAll += dhAll0/Nall*Nmult
				maghAll += maghAll0/Nall*Nmult
				...
				NobsPrsa = len(prsaObs.index)
				if (Nobs >= Nlim):
					m1cObs0, m1b = np.histogram(obs["m1"], bins=mbins)
					qhObs0, qb = np.histogram(obs["m2"]/obs["m1"], bins=qbins)
					ehObs0, eb = np.histogram(obs["e"], bins=ebins)
					lphObs0, lpb = np.histogram(np.ma.log10(obs["p"].values).filled(-999), bins=lpbins)
					dhObs0, db = np.histogram(obs["d"], bins=dbins)
	...
	saveHist(qcAll, qhObs, qhRec, qb, 'q (m2/m1)', 'EBLSST_qhist')
