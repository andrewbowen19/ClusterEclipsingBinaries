...
# Column names 1: (output files)
"""
OpSimID,OpSimRA,OpSimDec,
NOpSimObs_u,NOpSimObs_g,NOpSimObs_r,NOpSimObs_i,NOpSimObs_z,NOpSimObs_y,
clusterName,clusterMass,clusterDistance,clusterMetallicity,clusterAge,clusterRhm,clusterVdisp

"""
# Column names 2: (output files)
"""
p,m1,m2,r1,r2,k1,k2,e,i,d,nobs,Av,[M/H],appMagMean_r,maxDeltaMag,
deltaMag_r,mag_failure,incl_failure,period_failure,radius_failure,
...
def saveHist(histAll, histObs, histRec, bin_edges, xtitle, fname):
	...
	ax1.set_xlabel(xtitle, fontsize = 24)
	ax1.set_title('All Clusters', fontsize = 24)
	#CDF
	#cdfAll = []
	#cdfObs = []
	#cdfRec = []
	...
	f.savefig('/projects/p30137/abowen/CEB/plots/' + fname + '.pdf',format='pdf', bbox_inches = 'tight')

	#write to a text file
	with open('/projects/p30137/abowen/CEB/data_files/ebData/'+fname+'.csv','w') as f:
...
# Also, this weights those near the galactic plane sooo highly (and these are usually poorly recovered), that the resulting histograms are VERY noisy (since we're basically just looking at a few fields new to galactic plane)
		...
		print('Nobs values:', rec['nobs'].values)	
			#Appending to list for scatter plot
		if rec['nobs'].values.size != 0:
			observationsN.append(rec['nobs'].values[0])
		elif rec['nobs'].values.size == 0:
			observationsN.append(0)
	...
	saveHist(np.insert(m1hAll,0,0), np.insert(m1hObs,0,0), np.insert(m1hRec,0,0), m1b, 'm1 (M_\odot\)', 'EBLSST_m1hist')
	saveHist(np.insert(qhAll,0,0), np.insert(qhObs,0,0), np.insert(qhRec,0,0), qb, 'q (m2/m1)', 'All Clusters' + '_q')
	saveHist(np.insert(ehAll,0,0), np.insert(ehObs,0,0), np.insert(ehRec,0,0), eb, 'e', 'EBLSST_ehist')
	saveHist(np.insert(lphAll,0,0), np.insert(lphObs,0,0), np.insert(lphRec,0,0), lpb, 'log(P [days])', 'All Clusters' + '_l-p')
	...
	f.savefig('/projects/p30137/abowen/CEB/plots/mollweide_pct.pdf',format='pdf', bbox_inches = 'tight')
	...
	f.savefig('/projects/p30137/abowen/CEB/plots/mollweide_N.pdf',format='pdf', bbox_inches = 'tight')
...
#Printing percentages:
	print('percent recovered/all:', np.sum(fileRecN)/np.sum(fileN))

print(len(observationsN),len(recFrac))

# Nobs versus recoverability scatter plot - one for each cluster
f,ax = plt.subplots(figsize = (8,5))

ax.scatter(observationsN, recFrac)
ax.set_xlabel('$N_{obs}$')
ax.set_ylabel('Cluster Recoverability')
#ax.set_ylim((0,2))
f.savefig('/projects/p30137/abowen/CEB/plots/rec-scatter.pdf')
